...
export default function Home() {
  ...
  const updateText = useCallback(
    debounce(async (_id: string, value: string): Promise<void> => {
      setIsUpdating(_id);
      try {
        await updateHighlightText(_id, value);
      } catch (error) {
  ...
  return (
    <div className="wrapperContainer">
      ...
      {isLoading ? (
        ...
        <div>
          {highlights.map(({ _id, value }, index) => (
            <div
              key={_id}
              draggable
            ...
            >
              <span className="drag-handle">
                ...
                <Image src={'icon.svg'} alt="icon" height={20} width={10} />
              </span>
              <input
                type="text"
                value={value}
              ...
              />
              <button
                onClick={() => deleteHighlight(_id)}
                className="delete-button"
                disabled={isDeleting === _id || isActionInProgress}
              >
                {isDeleting === _id ? (
                  'Deleting...'
                ) : (
